      ...
      'Mod-d': () => {
        try {
          ...
          if (targetNode && targetDepth > 0) {
            ...
            return this.editor.commands.deleteRange({ from: pos, to: pos + nodeSize })
          }
...
// 可调整大小的图片扩展
export const createResizableImage = (isMobile: boolean = false) => {
  return Image.extend({
    addAttributes() {
      return {
        ...this.parent?.(),
...
export function createBaseExtensions(config: EditorExtensionsConfig = {}) {
  const {
    placeholder = '开始写作...',
  ...
  if (enableResizableImage) {
    extensions.push(createResizableImage(isMobile).configure({
      allowBase64: true,
  ...
  if (enableWikiLink) {
    extensions.push(
      WikiLink.configure({
        onLinkClick: wikiLinkOnClick || ((target: string) => console.log('Wiki link clicked:', target)),
      }),
      Extension.create({

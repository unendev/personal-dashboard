...
interface ActiveQuestOption {
  id: string
  title: string
}

interface CreateLogFormProps {
  activeQuests: ActiveQuestOption[]
}
...
export default function CreateLogForm({ activeQuests }: CreateLogFormProps) {
  ...
  const defaultTimestampValue = useMemo(() => {
    const now = new Date()
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      // 若未提供 categories，则提供一个空数组字符串，满足 Server Action 的解析
      ...
      if (formRef.current) {
        ...
        // 重置时间为当前
        const timestampInput = formRef.current.querySelector(
        ...
        if (timestampInput) {
          ...
          const tzOffsetMinutes = now.getTimezoneOffset()
          const local = new Date(now.getTime() - tzOffsetMinutes * 60000)
          timestampInput.value = local.toISOString().slice(0, 16)
        }
      }
    } catch (error) {
    ...
    <form ref={formRef} action={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            id="timestamp"
            name="timestamp"
            defaultValue={defaultTimestampValue}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-500"
          />
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-2 rounded-md font-medium disabled:opacity-50"
        >
